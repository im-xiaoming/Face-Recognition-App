...
def estimate_pose_and_quality(aligned_face: np.ndarray, face_info, expected_pose: str | None = None) -> dict:
    ...
    if det_score < MIN_DET_SCORE:
        ...
        results_dict["reject_reason"] = f"Linh diện chưa rõ nét (det_score={det_score:.2f})"
        return results_dict
    ...
    if img_area > 0:
        ...
        if face_area_ratio < MIN_FACE_AREA_RATIO:
            results_dict["reject"] = True
            results_dict["reject_reason"] = f"Linh diện quá nhỏ (ratio={face_area_ratio:.3f})"
            return results_dict
    ...
    if blur_score < MIN_BLUR_SCORE:
        results_dict["reject"] = True
        results_dict["reject_reason"] = f"Linh ảnh quá mờ (blur={blur_score:.1f})"
        return results_dict

    if brightness < MIN_BRIGHTNESS or brightness > MAX_BRIGHTNESS:
        results_dict["reject"] = True
        results_dict["reject_reason"] = f"Linh quang chưa đạt (brightness={brightness:.1f})"
        return results_dict
    ...
    if expected_pose and expected_pose in POSE_LIMITS:
        ...
        if not (limits['yaw'][0] <= yaw <= limits['yaw'][1]):
            ...
            results_dict["reject_reason"] = (
                f"Góc tả hữu chưa khớp '{expected_pose}' (yaw={yaw:.1f})"
            )
            return results_dict
        if not (limits['pitch'][0] <= pitch <= limits['pitch'][1]):
            results_dict["reject"] = True
            results_dict["reject_reason"] = (
                f"Góc cúi ngẩng chưa khớp '{expected_pose}' (pitch={pitch:.1f})"
            )
            return results_dict
        if not (limits['roll'][0] <= roll <= limits['roll'][1]):
            results_dict["reject"] = True
            results_dict["reject_reason"] = (
                f"Góc nghiêng chưa khớp '{expected_pose}' (roll={roll:.1f})"
            )
            return results_dict
